...
class AuthService:
    ...
    def login(email, password):
        ...
        # Try to find user by email first, then by role_id
        user = User.query.filter_by(email=email).first()
        if not user:
            user = User.query.filter_by(role_id=email).first()
        if not user:
            user = User.query.filter_by(minor_role_id=email).first()
        
        if not user or not user.check_password(password):
            logger.warning("Login failed for identifier=%s: invalid credentials", email)
            raise UnauthorizedError("Invalid email/ID or password")
