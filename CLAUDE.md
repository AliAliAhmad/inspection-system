...
| 2026-03-01 | Work Planning: (1) Dark mode toggle hidden on /admin/work-planning only (MainLayout.tsx conditional render); (2) Publish Plan button added to Bar 2 right side — visible only for draft plans, Modal.confirm → publishMutation; (3) Auto-Schedule verified working — proper loading state, success/error feedback; (4) ViewToggle moved from Bar 2 → Bar 1 next to week nav; (5) Inspection jobs filtered out of atRiskJobs useMemo; (6) At-Risk drawer tabbed — adds Overdue tab (non-inspection jobs with overdue_value > 0); (7) Equipment name used everywhere instead of serial_number across WorkPlanningPage, TimelineJobBlock, JobsPool, GanttChartView, AIRatingSuggestions, DailyReviewForm, DailyReviewPage, WorkPlanDayPage, AllEngineerJobsPage, AllSpecialistJobsPage, DefectsPage, MyWorkPlanPage; (8) fmtHours helper added — rounds all hour displays to 1 decimal; TSC 0 errors | ✅ Done |
...
| 2026-03-02 | Fix: assessment/verdict "Missing N required answers" false error — `_validate_completeness` now applies same equipment sub-type filter used by the wizard, so questions filtered out at display time are no longer required during validation (`inspection_service.py` line 499-508) | ✅ Done |
| 2026-03-02 | Work Planning defect pool bugs: (1) fmtHours crash — made null-safe (h ?? 0).toFixed(1) + guarded totalHours reduce with || 0; (2) defect stays in pool — backend get_available_jobs now excludes defects already in current plan via WorkPlanJob subquery | ✅ Done |
| 2026-03-01 | Work Planning: fix inspection chip grouping — revert eqType key back to equipment_type first (was incorrectly using equipment.name which is unique per unit, causing N individual 1× rows instead of one grouped "3× RS500" chip); popover eqName still shows individual unit names | ✅ Done |
| 2026-03-01 | Work Planning: inspection calendar chip tooltip — popover now shows equipment name (extracted from description for inspection jobs) instead of job number (#56); inspection grouping key also uses equipment name so same equipment types group correctly | ✅ Done |
| 2026-03-01 | Work Planning: inspection berth auto-assignment — backend add_job now overrides frontend-passed berth with assignment.berth for inspection jobs, so East-assigned inspections always land in east column and West-assigned in west column | ✅ Done |
