...
export function useNotificationAlerts({ user, navigate }: UseNotificationAlertsOptions) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      notificationsApi.list({ unread_only: true, per_page: 5 }).then((r) => r.data),
    refetchInterval: 15_000, // Check every 15s for faster detection
    enabled: !!user,
  });
